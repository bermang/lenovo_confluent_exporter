...
The exporter uses the Confluent client to query nodes sensors and health status.
Sensors are defined per group under the groups statement. each group entry is defined by the group name and list of sensors to collect. 
While nodes can be in multiple groups, the exporter will perform only one collection per node per sensor 
